...
def parse_args():
  ...
  parser.add_argument('-a', '--account', type=str.lower, dest='account', choices=['dev', 'evt', 'prod'], required=True, help='Define account in which to create resources.')
  ...
  parser.add_argument('-V', '--vpc', type=str.lower, dest='vpc', choices=['poc', 'default', 'uat', 'website', 'prod', 'dev'], required=True, help='Define the VPC in which to create resources.')
  parser.add_argument('-E', '--environment', type=str.lower, dest='environment', choices=['dev', 'uat', 'preprod', 'prod', 'poc', 'test', 'stag'], required=True, help='Define environment in which to create resources.')
  parser.add_argument('-p', '--pod', type=str.lower, dest='pod', choices=['pod0', 'pod1', 'pod2', 'pod3', 'pod4', 'pod5', 'pod6', 'pod7', 'testpod'], required=True, help='Define pod in which to create resources.')
  parser.add_argument('-r', '--role', type=str.lower, dest='role', choices=['web', 'app', 'api', 'int', 'ext', 'plat', 'mgmt',  'test', 'webhelp', 'status'], required=True, help='Define type of instance to create.')
  ...
  build_args.config_dir = build_args.config_dir if build_args.config_dir else os.environ['HOME'] + '/home/terraform'
...
class terraform_build:
  ...
  def define_build(self):
    ...
    if self.build_args['elb']:
      if self.build_args['role'] in ['app', 'api']:
